...
#include <linux/usb.h>
#define USBLED_VENDOR_ID	0xaaaa
#define USBLED_PRODUCT_ID	0x8816
						 ...
						 const char *buf, size_t count)
{
	...
	u8 val;
	int error, retval;
	_dev_info(&intf->dev, "led_store() function is called.\n");
	...
	if (val == 1 || val == 2 || val == 3)
		_dev_info(&led->udev->dev, "led = %d\n", led->led_number);
	else
	{
		_dev_info(&led->udev->dev, "unknown led %d\n", led->led_number);
		retval = -EINVAL;
		return retval;
	}
...
static int led_probe(struct usb_interface *interface, const struct usb_device_id *id)
{
	...
	int retval = -ENOMEM;
	_dev_info(&interface->dev, "led_probe() function is called.\n");
...
static void led_disconnect(struct usb_interface *interface)
{
	...
	kfree(dev);
	_dev_info(&interface->dev, "USB LED now disconnected\n");
}
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Vikas Kumar");
MODULE_DESCRIPTION("This is the usb_led using misc framework");
